...
def lambda_handler(event, context):
    ...
    This handler uses the single-user rotation scheme to rotate a
    Redshift user credential. This rotation scheme logs into the
    database as the user and rotates the user's own password,
    immediately invalidating the user's previous password.

    The Secret SecretString is expected to be a JSON string with the
    following format:
    {
    ...
    if "AWSCURRENT" in versions[token]:
        logger.info(
            "Secret version %s already AWSCURRENT for secret %s."
            % (token, arn)
        )
        return
    elif "AWSPENDING" not in versions[token]:
        logger.error(
            "Secret version %s not set as AWSPENDING "
            "for rotation of secret %s."
            % (token, arn)
        )
        raise ValueError(
            "Secret version %s not set as AWSPENDING "
    ...
    if step == "createSecret":
        ...
        logger.error(
            "lambda_handler: Invalid step parameter %s for secret %s"
            % (step, arn)
        )
        raise ValueError(
            "Invalid step parameter %s for secret %s"
            % (step, arn)
        )
...
def create_secret(service_client, arn, token):
    ...
    This method first checks for the existence of a secret for the
    passed in token. If one does not exist, it will generate a new
    secret and put it with the passed in token.
...
def set_secret(service_client, arn, token):
    ...
    and returns on success. If that fails, it tries to login with the
    AWSCURRENT and AWSPREVIOUS secrets. If either one succeeds, it sets
    the AWSPENDING password as the user password in the database. Else,
    ...
    Raises:
        ...
        ValueError: If the secret is not valid JSON or valid credentials
        are found to login to the database
    ...
    if conn:
        conn.close()
        logger.info(
            "setSecret: AWSPENDING secret is already set as password in "
            "Redshift DB for secret arn %s.",
            arn
        )
        return
    ...
    if current_dict['username'] != pending_dict['username']:
        logger.error(
            "setSecret: Attempting to modify user %s "
            "other than current user %s",
    ...
    if current_dict['host'] != pending_dict['host']:
        logger.error(
            "setSecret: Attempting to modify user for host %s "
            "other than current host %s"
    ...
    if not conn and previous_dict:
        ...
        if previous_dict['username'] != pending_dict['username']:
            logger.error(
                ("setSecret: Attempting to modify user %s "
                 "other than previous valid user %s")
                % (pending_dict['username'], previous_dict['username'])
            )
            raise ValueError(
                ("Attempting to modify user %s "
                 "other than previous valid user %s")
                % (pending_dict['username'], previous_dict['username'])
            )
        if previous_dict['host'] != pending_dict['host']:
            logger.error(
                ("setSecret: Attempting to modify user for host %s "
                 "other than previous host %s")
                % (pending_dict['host'], previous_dict['host'])
            )
            raise ValueError(
                ("Attempting to modify user for host %s "
                 "other than previous host %s")
                % (pending_dict['host'], previous_dict['host'])
            )
    ...
    if not conn:
        logger.error(
            "setSecret: Unable to log into database with previous, current,"
            " or pending secret of secret arn %s" % arn
        )
        raise ValueError(
            "Unable to log into database with previous, current, or pending "
    ...
    try:
        with conn.cursor() as cur:
            ...
            pending_password = pending_dict['password']
            cur.execute(alter_role + " WITH PASSWORD %s", (pending_password,))
            conn.commit()
            logger.info(
                ("setSecret: Successfully set password for user %s "
                 "in Redshift DB for secret arn %s.")
...
def test_secret(service_client, arn, token):
    ...
    This method tries to log into the database with the secrets staged
    with AWSPENDING and runs a permissions check to ensure the user has
    ...
    if conn:
        ...
        # Uncomment or modify the lines below to tailor these validations
        # to your needs.
        try:
            with conn.cursor() as cur:
        ...
        logger.info(
            "testSecret: Successfully signed into Redshift DB with AWSPENDING "
            "secret in %s.",
            arn,
        )
        return
        ...
        logger.error(
            "testSecret: Unable to log into database with pending secret of "
            "secret ARN %s" % arn
        )
        raise ValueError(
            "Unable to log into database with pending secret of "
            "secret ARN %s" % arn
        )
...
def get_connection(secret_dict):
    ...
    This helper function tries to connect to the database
    grabbing connection info from the secret dictionary. If
    successful, it returns the connection, else None
...
def get_secret_dict(service_client, arn, stage, token=None):
    ...
    This helper function gets credentials for the arn and stage passed in
    and returns the dictionary by parsing the JSON string
    ...
    if 'engine' not in secret_dict or secret_dict['engine'] != 'redshift':
        raise KeyError(
            "Database engine must be 'redshift' to use this rotation lambda"
        )
...
def get_environment_bool(variable_name, default_value):
    ...
    Args:
        ...
        default_value (bool): The result will fallback to the default_value
            when the environment variable with the given name doesn't exist.

    Returns:
        bool: True when the content of environment variable contains either
        'true', '1', 'y' or 'yes'
    """
...
def get_random_password(service_client):
    ...
    The generator loads parameters that affect the content of the resulting
    password from environment variables. When an environment variable is
    missing, sensible defaults are chosen.
    ...
    Redshift requires password to have at least one lower, one upper case
    and one number character. Because of that following options are omitted:
        - EXCLUDE_UPPERCASE
        - EXCLUDE_LOWERCASE
        - EXCLUDE_NUMBERS
